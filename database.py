...
#  La variable DATABASE_URL est automatiquement définie
#  par Railway dans l'environnement de production.
...
if DATABASE_URL:
    try:
        ...
        log.info("✅ EMPIRE CONNECTÉ : Base de données Railway en ligne !")
        print("✅ EMPIRE CONNECTÉ : La base de données Railway est en ligne !")
    except Exception as e:
        log.error(f"❌ ERREUR CONNEXION RAILWAY : {e}")
        print(f"❌ ERREUR DE CONNEXION : {e}")
        pg_pool = None
else:
    # ── Fallback : connexion manuelle via config.py (développement local)
    try:
        ...
        log.info("✅ Connexion PostgreSQL locale établie.")
        print("✅ Connexion PostgreSQL locale établie.")
    except Exception as e:
...
if REDIS_URL:
    # Redis Railway (si disponible)
    redis_client = redis.Redis.from_url(REDIS_URL, decode_responses=True)
    log.info("✅ Redis Railway connecté.")
else:
    # Redis local
    ...
    log.info("✅ Redis local connecté.")

# ═══════════════════════════════════════════════════════
#  CONTEXT MANAGER — Connexion thread-safe depuis le pool
# ═══════════════════════════════════════════════════════
@contextmanager
def pg():
    if pg_pool is None:
        raise RuntimeError("❌ Pool PostgreSQL non initialisé. Vérifiez DATABASE_URL.")
...
# ═══════════════════════════════════════════════════════
PIN_MAX_FAILURES    = 3
PIN_LOCKOUT_SECONDS = 3600   # 1 heure
...
def check_pin_lockout(uid: int) -> int:
    """Retourne les secondes restantes de verrouillage, 0 si pas verrouillé."""
    ttl = r().ttl(_pin_lock_key(uid))
    return max(ttl, 0)

def record_pin_failure(uid: int) -> int:
    """Incrémente le compteur d'échecs. Verrouille après 3 échecs.
    Retourne le nombre de tentatives restantes (0 = vient d'être verrouillé)."""
...
def verify_pin(uid: int, pin: str) -> dict:
    """
    Vérifie le PIN avec protection anti-brute-force.
    Retourne : {ok: bool, message: str, lockout_secs: int}
    """
    u = get_user(uid)
    if not u:
...
def fill_history_if_empty():
    """Pré-remplit l'historique si vide (simulation de veille)."""
...
# ═══════════════════════════════════════════════════════
#  ÉTAT SERVEUR (ouverture/fermeture du site)
# ═══════════════════════════════════════════════════════
...
def get_caisse() -> float:
    ...
    with pg() as conn:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
    solde = float(row[0]) if row else 500_000.0
